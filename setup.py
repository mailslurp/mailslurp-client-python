...
To use SMTP clients first create an SMTP type inbox and use the inbox controller `get_imap_smtp_access` method on the [inbox controller](https://python.mailslurp.com/api/inbox_controller_api.html) to obtain the username, password, port and host.
...
To send attachments first use the [attachment controller](https://python.mailslurp.com/api/attachment_controller_api.html) to upload a file or byte stream. Then use the attachment ID returned with subsequent send calls.
