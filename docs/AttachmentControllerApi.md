...
> list[str] download_attachment_as_bytes(attachment_id)
...
before = '2013-10-20T19:20:30+01:00' # datetime | Filter by created at before the given timestamp (optional)
    ...
    try:
        # Get email attachments
        api_response = api_instance.get_attachments1(page=page, size=size, sort=sort, file_name_filter=file_name_filter, since=since, before=before)
        pprint(api_response)
    except ApiException as e:
        print("Exception when calling AttachmentControllerApi->get_attachments1: %s\n" % e)
```
