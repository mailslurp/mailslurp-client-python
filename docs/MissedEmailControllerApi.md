...
with mailslurp_client.ApiClient(configuration) as api_client:
    ...
    try:
        ...
        print("Exception when calling MissedEmailControllerApi->restore_missed_emails: %s\n" % e)
```
...
> MissedEmailDto wait_for_nth_missed_email(index, inbox_id=inbox_id, timeout=timeout, since=since, before=before)
