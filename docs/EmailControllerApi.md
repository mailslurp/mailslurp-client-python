...
> EmailTextLinesResult get_email_text_lines(email_id, decode_html_entities=decode_html_entities)
...
decode_html_entities = True # bool | Decode HTML entities (optional)
    ...
    try:
        # Parse and return text from an email, stripping HTML and decoding encoded characters
        api_response = api_instance.get_email_text_lines(email_id, decode_html_entities=decode_html_entities)
        pprint(api_response)
    except ApiException as e:
