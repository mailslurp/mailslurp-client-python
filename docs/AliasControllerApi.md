...
verification_token = 'verification_token_example' # str | verificationToken

    try:
        # Get validation result from alias verification
        api_response = api_instance.get_alias_verification(alias_id, email_address, verification_token)
        pprint(api_response)
    except ApiException as e:
        print("Exception when calling AliasControllerApi->get_alias_verification: %s\n" % e)
```
