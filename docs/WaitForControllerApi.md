...
with mailslurp_client.ApiClient(configuration) as api_client:
    ...
    wait_for_conditions = mailslurp_client.WaitForConditions() # WaitForConditions | Conditions to apply to emails that you are waiting for (optional)
...
> list[EmailPreview] wait_for_email_count(before=before, count=count, delay=delay, inbox_id=inbox_id, since=since, sort=sort, timeout=timeout, unread_only=unread_only)
...
unread_only = False # bool | Optional filter for unread only (optional) (default to False)
    ...
    try:
        # Wait for and return count number of emails. Hold connection until inbox count matches expected or timeout occurs
        api_response = api_instance.wait_for_email_count(before=before, count=count, delay=delay, inbox_id=inbox_id, since=since, sort=sort, timeout=timeout, unread_only=unread_only)
        pprint(api_response)
    except ApiException as e:
...
unread_only = False # bool | Optional filter for unread only. (optional) (default to False)
    ...
    try:
        # Fetch inbox's latest email or if empty wait for an email to arrive
        api_response = api_instance.wait_for_latest_email(before=before, delay=delay, inbox_id=inbox_id, since=since, sort=sort, timeout=timeout, unread_only=unread_only)
        pprint(api_response)
    except ApiException as e:
...
> list[EmailPreview] wait_for_matching_emails(match_options, before=before, count=count, delay=delay, inbox_id=inbox_id, since=since, sort=sort, timeout=timeout, unread_only=unread_only)
...
before = '2013-10-20T19:20:30+01:00' # datetime | Filter for emails that were received before the given timestamp (optional)
count = 56 # int | Number of emails to wait for. Must be greater or equal to 1 (optional)
delay = 56 # int | Max milliseconds delay between calls (optional)
inbox_id = 'inbox_id_example' # str | Id of the inbox we are fetching emails from (optional)
since = '2013-10-20T19:20:30+01:00' # datetime | Filter for emails that were received after the given timestamp (optional)
sort = 'sort_example' # str | Sort direction (optional)
...
unread_only = False # bool | Optional filter for unread only (optional) (default to False)
    ...
    try:
        # Wait or return list of emails that match simple matching patterns
        api_response = api_instance.wait_for_matching_emails(match_options, before=before, count=count, delay=delay, inbox_id=inbox_id, since=since, sort=sort, timeout=timeout, unread_only=unread_only)
        pprint(api_response)
    except ApiException as e:
...
> Email wait_for_matching_first_email(match_options, before=before, delay=delay, inbox_id=inbox_id, since=since, sort=sort, timeout=timeout, unread_only=unread_only)
...
unread_only = False # bool | Optional filter for unread only (optional) (default to False)
    ...
    try:
        # Wait for or return the first email that matches provided MatchOptions array
        api_response = api_instance.wait_for_matching_first_email(match_options, before=before, delay=delay, inbox_id=inbox_id, since=since, sort=sort, timeout=timeout, unread_only=unread_only)
        pprint(api_response)
    except ApiException as e:
 ...
 **before** | **datetime**| Filter for emails that were received before the given timestamp | [optional] 
 **delay** | **int**| Max milliseconds delay between calls | [optional] 
 **inbox_id** | [**str**]()| Id of the inbox we are matching an email for | [optional] 
 **since** | **datetime**| Filter for emails that were received after the given timestamp | [optional] 
 **sort** | **str**| Sort direction | [optional] 
 **timeout** | **int**| Max milliseconds to wait | [optional] 
 **unread_only** | **bool**| Optional filter for unread only | [optional] [default to False]
...
unread_only = False # bool | Optional filter for unread only (optional) (default to False)
    ...
    try:
        # Wait for or fetch the email with a given index in the inbox specified. If index doesn't exist waits for it to exist or timeout to occur.
        api_response = api_instance.wait_for_nth_email(before=before, delay=delay, inbox_id=inbox_id, index=index, since=since, sort=sort, timeout=timeout, unread_only=unread_only)
        pprint(api_response)
    except ApiException as e:
