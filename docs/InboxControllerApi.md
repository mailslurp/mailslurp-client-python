...
# **get_emails**
> list[EmailPreview] get_emails(inbox_id, alias_for_limit__assessed_first_before_assessing_any_passed_limit_=alias_for_limit__assessed_first_before_assessing_any_passed_limit_, limit=limit, sort_the_results_by_received_date_and_direction_asc_or_desc=sort_the_results_by_received_date_and_direction_asc_or_desc, retry_timeout=retry_timeout, delay_timeout=delay_timeout, min_count=min_count, unread_only=unread_only, before=before, since=since)
...
since = '2013-10-20T19:20:30+01:00' # datetime | Exclude emails received before this ISO 8601 date time (optional)
    ...
    try:
        # Get emails in an Inbox. This method is not idempotent as it allows retries and waits if you want certain conditions to be met before returning. For simple listing and sorting of known emails use the email controller instead.
        api_response = api_instance.get_emails(inbox_id, alias_for_limit__assessed_first_before_assessing_any_passed_limit_=alias_for_limit__assessed_first_before_assessing_any_passed_limit_, limit=limit, sort_the_results_by_received_date_and_direction_asc_or_desc=sort_the_results_by_received_date_and_direction_asc_or_desc, retry_timeout=retry_timeout, delay_timeout=delay_timeout, min_count=min_count, unread_only=unread_only, before=before, since=since)
        pprint(api_response)
    except ApiException as e:
